...
export default function QuoteExporter() {
  ...
  const generateCSV = () => {
    const headers = ['Product', 'Brand', 'Quantity', 'Unit Price', 'Total Price']
  ...
  const totalDiscount = (state.customer?.discount_group?.discount_percentage || 0) + state.additionalDiscount
                    ...
                    <tbody>
                      {state.products.map((product) => (
                        <tr key={product.id} className="border-b border-slate-100">
                          ...
                          <td className="py-2 text-right text-slate-700">{product.quantity}</td>
                          <td className="py-2 text-right text-slate-700 mono">${product.price.toFixed(2)}</td>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={exporting}
                className="w-full px-4 py-2 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors disabled:opacity-50"
              >
                {exporting ? 'Completing...' : 'Complete Quote'}
              </button>
              <button className="w-full px-4 py-2 text-sm bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors">
                Save to Dashboard
              </button>
              <button className="w-full px-4 py-2 text-sm bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors">
                Create New Quote
              </button>
            </div>
          </div>
