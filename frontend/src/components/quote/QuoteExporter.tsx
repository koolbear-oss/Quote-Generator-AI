...
export const QuoteExporter = forwardRef(function QuoteExporter(props, ref) {
  ...
  const totalDiscount = state.products.reduce((sum, product) => {
    const discountPercentage = product.discount_percentage || 0;
    const unitPrice = product.gross_price || product.price || 0;
    const quantity = product.quantity || 1;
    return sum + (unitPrice * quantity * discountPercentage / 100);
  }, 0);
  
  const volumeDiscountAmount = grossTotal * (state.volumeDiscount / 100 || 0);
  const additionalDiscountAmount = grossTotal * (state.additionalDiscount / 100 || 0);
  
  const totalProjectDiscount = totalDiscount + volumeDiscountAmount + additionalDiscountAmount;
  const totalDiscountPercentage = grossTotal > 0 ? (totalProjectDiscount / grossTotal) * 100 : 0;
  const netTotal = grossTotal - totalProjectDiscount;
  ...
  // Debug function to check authentication status (kept for completeness)
  ...
  const completeQuote = async () => {
    try {
      ...
      // CRITICAL FIX: Get the DAS Solution UUID from the name
      let dasSolutionId;
      
      // Check if state.dasSolution.id is already a UUID
      const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      if (state.dasSolution.id && uuidPattern.test(state.dasSolution.id)) {
        dasSolutionId = state.dasSolution.id;
      } else {
        // Fetch the UUID for this solution name
        const { data: solutionData, error: solutionError } = await supabase
          .from('das_solutions')
          .select('id')
          .eq('name', state.dasSolution.name)
          .single();
          
        if (solutionError) {
          console.error("Error finding DAS solution:", solutionError);
          
          // Fallback: Insert a new solution record
          const { data: newSolution, error: insertError } = await supabase
            .from('das_solutions')
            .insert({ name: state.dasSolution.name })
            .select()
            .single();
            
          if (insertError) {
            throw new Error(`Could not create solution: ${insertError.message}`);
          }
          
          dasSolutionId = newSolution.id;
        } else {
          dasSolutionId = solutionData?.id;
        }
      }
      
      if (!dasSolutionId) {
        throw new Error("Could not determine DAS solution ID");
      }
      ...
      const quoteData = {
        ...
        das_solution_id: dasSolutionId, // Use the retrieved UUID
        project_discount: totalDiscountPercentage || 0, // Store the combined discount
        total_discount_amount: totalProjectDiscount || 0,
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div>
            <p className="text-sm text-gray-500">Total Project Discount</p>
            <p className="font-semibold">{totalDiscountPercentage.toFixed(1)}%</p>
          </div>
        </div>
        ...
        <div className="border-t pt-4">
          ...
          {totalProjectDiscount > 0 && (
            <div className="flex justify-between items-center mb-2 text-green-600">
              ...
              <span>-${totalProjectDiscount.toFixed(2)}</span>
            </div>
          )}
